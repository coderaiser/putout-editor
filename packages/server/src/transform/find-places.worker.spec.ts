...
test('find-places worker: returns places array', async (t) => {
    const {places} = await findPlaces({fixture: 'var x = 1;', plugin: replaceVarWithConst});
    t.ok(Array.isArray(places));
    t.end();
});
...
test('find-places worker: finds multiple matches', async (t) => {
    const {places} = await findPlaces({fixture: 'var x = 1;\nvar y = 2;', plugin: replaceVarWithConst});
    t.equal(places.length, 2);
    t.end();
});

test('find-places worker: returns empty array on no match', async (t) => {
    const {places} = await findPlaces({fixture: 'const x = 1;', plugin: replaceVarWithConst});
...
test('find-places worker: place has rule string', async (t) => {
    const {places} = await findPlaces({fixture: 'var x = 1;', plugin: replaceVarWithConst});
    t.equal(typeof places[0].rule, 'string');
    t.end();
});
...
test('find-places worker: place has position.line number', async (t) => {
    const {places} = await findPlaces({fixture: 'var x = 1;', plugin: replaceVarWithConst});
    t.equal(typeof places[0].position.line, 'number');
    t.end();
});

test('find-places worker: place has position.column number', async (t) => {
    const {places} = await findPlaces({fixture: 'var x = 1;', plugin: replaceVarWithConst});
    t.equal(typeof places[0].position.column, 'number');
    t.end();
});
...
test('find-places worker: does not modify fixture source', async (t) => {
    ...
    await findPlaces({fixture, plugin: replaceVarWithConst});
    t.ok(true);
    t.end();
});