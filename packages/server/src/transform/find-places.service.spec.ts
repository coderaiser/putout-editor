...
function createServiceWithRun(run: unknown) {
    ...
    (service as unknown as {pool: PoolWithRun}).pool = {run: run as () => Promise<unknown>};
    return service;
}
...
test('find-places service: documentation has workflow string', (t) => {
    const service = new FindPlacesService();
    t.equal(typeof service.documentation().workflow, 'string');
    t.end();
});
...
test('find-places service: findPlaces returns places array', async (t) => {
    const run = stub().resolves({places: [{rule: 'rule', message: 'use const', position: {line: 1, column: 0}}]}});
    const service = createServiceWithRun(run);
    const result = await service.findPlaces({fixture: 'var x = 1;', plugin: ''});
    t.ok(Array.isArray(result.places));
    t.end();
});

test('find-places service: returns  ̄400 on plugin_syntax error', async (t) => {
    const run = stub().rejects(Object.assign(new Error('bad'), {
        structured: {kind: 'plugin_syntax', message: 'bad', position: {line: 1, column: 5}},
    }));
    const service = createServiceWithRun(run);
    const [error] = await tryToCatch(service.findPlaces.bind(service, {fixture: 'x', plugin: ''});
    t.equal((error as {status?: number}).status, 400);
    t.end();
});

test('find-places service: returns  ̄422 on plugin_error', async (t) => {
