...
test('parserMiddleware: INIT triggers parse and dispatches SET_PARSE_RESULT with ast', async (t) => {
    ...
    babel._promise = Promise.resolve({
        parse: (code, opts) => makeMockParseResult(),
    });
    babel.parse = (_, code, opts) => makeMockParseResult();
    ...
    const types = nexted.map((a) => a.type);
    t.ok(types.includes('SET_PARSE_RESULT'));
    t.end();
});
...
test('parserMiddleware: no change skips parse', (t) => {
    ...
    // next(action) passes through, but no SET_PARSE_RESULT
    const types = nexted.map((a) => a.type);
    t.notOk(types.includes('SET_PARSE_RESULT'));
    t.end();
});
...
test('parserMiddleware: parse with settings filters import attributes', async (t) => {
    ...
    const state = makeBaseState({
        workbench: {
            ...makeBaseState().workbench,
            parserSettings: {
                plugins: ['jsx', 'importAssertions', ['importAttributes', {}]],
            },
        },
    });
    const store = {
...
test('parserMiddleware: state change between dispatch and resolve discards result', async (t) => {
    ...
    babel._promise = new Promise((r) => { resolveParse = r; });
    babel.parse = (_, code, opts) => makeMockParseResult();
