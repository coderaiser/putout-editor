...
import {astexplorer, revive, persist} from './reducers.js';
import {
    setCode,
    setCursor,
    setParser,
...
test('store: error defaults to null', (t) => {
    t.equal(getState(makeStore()).error, null);
    t.end();
});
...
test('store: clearError resets error to null', (t) => {
    const store = makeStore();
    dispatch(store, setError(new Error('fail')));
    dispatch(store, clearError());
    t.equal(getState(store).error, null);
    t.end();
});
...
test('store: setCode updates workbench.code', (t) => {
    const store = makeStore();
    dispatch(store, setCode({code: 'const x = 1', cursor: 0}));
...
test('store: cursor defaults to null', (t) => {
    t.equal(getState(makeStore()).cursor, null);
    t.end();
});

test('store: setCode with non-zero cursor updates cursor', (t) => {
    const store = makeStore();
    dispatch(store, setCode({code: 'x', cursor: 5}));
...
test('store: activeRevision defaults to null', (t) => {
    t.equal(getState(makeStore()).activeRevision, null);
    t.end();
});
...
test('store: setSnippet updates workbench.code from revision', (t) => {
    const store    = makeStore();
    const revision = makeRevision({getCode: () => 'const y = 2'});
    dispatch(store, setSnippet(revision));
...
test('store: setSnippet resets cursor to null', (t) => {
    ...
    t.equal(getState(store).cursor, null);
    t.end();
});
...
test('store: clearSnippet sets activeRevision to null', (t) => {
    ...
    t.equal(getState(store).activeRevision, null);
    t.end();
});
...
test('store: persist does not include cursor', (t) => {
    ...
    const persisted = persist({...state, cursor: 42});
...
test('store: revive sets workbench.initialCode from stored code', (t) => {
    const stored = revive({
        workbench: {
            code: 'const z = 3',
            keyMap: 'vim',
            transform: {code: '', transformer: 'putout'},
            parser: 'babel',
        },
...
test('store: revive sets workbench.transform.initialCode from stored transform code', (t) => {
    const stored = revive({
        workbench: {
            code: 'x',
            keyMap: 'vim',
            transform: {code: 'y', transformer: 'putout'},
            parser: 'babel',
        },
        showTransformPanel: true,
        parserSettings: {},
