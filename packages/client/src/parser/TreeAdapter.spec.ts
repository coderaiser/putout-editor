...
test('TreeAdapter: treeAdapterFromParseResult: walkNode does not throw for null treeAdapter', async (t) => {
    const adapter = treeAdapterFromParseResult({
        treeAdapter: null,
    }, {});
...
const makeBabelLikeAdapter = () => treeAdapterFromParseResult({
    treeAdapter: {
        ...
        options: {
            ...
            nodeToName: (node) => ((node as Record<string, unknown>).type as string),
            walkNode: function*(node) {
                for (const prop in node as object)
                    yield {
                        value: (node as Record<string, unknown>)[prop],
                        key: prop,
                    };
            },
        },
