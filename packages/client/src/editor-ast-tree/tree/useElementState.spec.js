...
test('useElementState: open is false by default at level 1', (t) => {
    ...
    t.equal(result.open, false);
    t.end();
});

test('useElementState: open is true at level 0', (t) => {
    let result;
    captureState(makeProps({level: 0}), makeAdapter(), (state) => {
        result = state;
    });
    t.equal(result.open, true);
    t.end();
});
...
test('useElementState: open is true when value is in focusPath as non-leaf', (t) => {
    ...
    let result;
    captureState(makeProps({value, focusPath: [value, leaf]}), makeAdapter(), (state) => {
        result = state;
    });
    t.equal(result.open, true);
    t.end();
...
test('useElementState: open is false when value is leaf in focusPath', (t) => {
    ...
    let result;
    captureState(makeProps({value, focusPath: [value]}), makeAdapter(), (state) => {
        result = state;
    });
    t.equal(result.open, false);
    t.end();
...
test('useElementState: initial value matches props.value', (t) => {
    const value = {type: 'Identifier'};
    let result;
    captureState(makeProps({value}), makeAdapter(), (state) => {
        result = state;
    });
...
test('useElementState: updates value when props change', async (t) => {
    const first = {type: 'Identifier'};
    const second = {type: 'Literal'};
    let result;

    const {rerender} = render(
        <TestHook props={makeProps({value: first})} adapter={makeAdapter()} onState={(state) => {
            result = state;
        }}/>,
    );
    ...
    await act(async () => {
        rerender(
            <TestHook props={makeProps({value: second})} adapter={makeAdapter()} onState={(state) => {
                result = state;
            }}/>,
        );
    });
