...
test('useHighlight: onMouseLeave clears highlightRange', (t) => {
    ...
    const store = configureStore({
        reducer: putoutEditor,
        preloadedState: {
            ...putoutEditor(undefined, {type: '@@INIT'}),
    ...
    t.equal(store.getState().highlightRange, null);
    t.end();
});
...
test('useHighlight: returns onMouseOver function', (t) => {
    ...
    render(
        <Provider store={configureStore({reducer: putoutEditor})}>
            <ProbeComponent/>
        </Provider>,
    );
    cleanup();
