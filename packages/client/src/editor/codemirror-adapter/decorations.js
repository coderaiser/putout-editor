        ...
        for (const effect of transaction.effects) {
            if (effect.is(setLineEffect)) {
                ...
                decorations = decorations.update({
                    add: [Decoration.line({class: cls}).range(from)],
                    sort: true,
                });
            }
            ...
            if (effect.is(clearLineEffect)) {
                const {line, cls} = effect.value;
                decorations = decorations.update({
                    filter: (from, _, decoration) =>
                        !(decoration.spec.class === cls &&
                          transaction.state.doc.lineAt(from).number === line + 1),
                });
            }
        }
...
export function markText(view, from, to, {className}) {
    const decoration = Decoration.mark({class: className}).range(
        positionToOffset(view.state.doc, from),
        positionToOffset(view.state.doc, to),
    );
    view.dispatch({effects: setMarkEffect.of(decoration)});
    return {clear: () => view.dispatch({effects: clearMarkEffect.of(null)})};
}

export function addLineClass(view, line, _where, className) {
    view.dispatch({effects: setLineEffect.of({line, cls: className})});
}

export function removeLineClass(view, line, _where, className) {
    view.dispatch({effects: clearLineEffect.of({line, cls: className})});
}