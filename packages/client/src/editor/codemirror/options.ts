...
export function languageExtension(mode: EditorMode | {name: EditorMode}): Extension {
...
export type OptionValue = EditorTheme | KeyMap | EditorMode | {name: EditorMode};

export function setOption(
    view: EditorView & {
        _themeCompartment: import('@codemirror/state').Compartment;
        _keymapCompartment: import('@codemirror/state').Compartment;
        _langCompartment: import('@codemirror/state').Compartment;
    },
    key: OptionKey,
    value: OptionValue,
): void {
    ...
    if (key === 'mode')
        return view.dispatch({
            effects: view._langCompartment.reconfigure(languageExtension(value as EditorMode | {name: EditorMode})),
        });
}