...
test('Editor: rerenders without error when value prop changes', async (t) => {
    const {container, rerender} = render(<Editor value="const x = 1"/>);
    await act(async () => {
        rerender(<Editor value="const y = 2"/>);
    });
    const result = container.querySelector('.editor');
    cleanup();
    t.ok(result);
...
test('Editor: rerenders without error when mode prop changes', async (t) => {
    const {container, rerender} = render(<Editor value="x" mode="javascript"/>);
    await act(async () => {
...
test('Editor: rerenders without error when keyMap prop changes', async (t) => {
    const {container, rerender} = render(<Editor value="x" keyMap="default"/>);
    await act(async () => {
...
test('Editor: rerenders without error when error prop changes', async (t) => {
    const {container, rerender} = render(<Editor value="x" error={null}/>);
    await act(async () => {
        rerender(<Editor value="x" error={{loc: {line: 2}, message: 'err'}}/>);
    });
    const result = container.querySelector('.editor');
    cleanup();
    t.ok(result);
...
test('Editor: clears previous error line when error prop changes to null', async (t) => {
    const {container, rerender} = render(
        <Editor value="x" error={{loc: {line: 1}, message: 'first'}}/>,
    );
    await act(async () => {
        rerender(<Editor value="x" error={null}/>);
    });
    const result = container.querySelector('.editor');
    cleanup();
    t.ok(result);
...
test('Editor: rerenders without error when highlightRange changes', async (t) => {
    const {container, rerender} = render(<Editor value="x" highlightRange={[0, 3]}/>);
    await act(async () => {
        rerender(<Editor value="x" highlightRange={[1, 5]}/>);
    });
    const result = container.querySelector('.editor');
    cleanup();
    t.ok(result);
...
test('Editor: clears mark when highlightRange changes to null', async (t) => {
    const {container, rerender} = render(<Editor value="x" highlightRange={[0, 3]}/>);
    await act(async () => {
        rerender(<Editor value="x" highlightRange={null}/>);
    });
    const result = container.querySelector('.editor');
    cleanup();
    t.ok(result);
...
test('Editor: uses posFromIndex prop when provided', async (t) => {
...
test('Editor: handles highlightRange with no valid positions gracefully', async (t) => {
...
test('Editor: error prop with lineNumber field renders without error', async (t) => {
    const {container} = render(<Editor value="x" error={{lineNumber: 1, message: 'err'}}/>);
    const result = container.querySelector('.editor');
    cleanup();
    t.ok(result);
    t.end();
...
test('Editor: error prop with line field renders without error', async (t) => {
    const {container} = render(<Editor value="x" error={{line: 1, message: 'err'}}/>);
    const result = container.querySelector('.editor');
    cleanup();
    t.ok(result);
    t.end();
...
test('Editor: calls onContentChange after value change', async (t) => {
    let received = null;
    const {container} = render(
        <Editor value="x" onContentChange={(args) => { received = args; }}/>,
    );
    await act(async () => {
...
test('Editor: onContentChange receives cursor field', async (t) => {
    ...
    cleanup();
    t.equal(typeof received?.cursor, 'number');
    t.end();
});
...
test('Editor: calls onActivity after cursor moves', async (t) => {
    let called = false;
    const {container} = render(
        <Editor value="hello world" onActivity={() => { called = true; }}/>,
    );
    await act(async () => {
        getCM(container).setCursor({line: 0, ch: 5});
...
test('Editor: onActivity receives a number', async (t) => {
    let received = null;
    const {container} = render(
        <Editor value="hello world" onActivity={(idx) => { received = idx; }}/>,
    );
    await act(async () => {
        getCM(container).setCursor({line: 0, ch: 3});
        await new Promise((r) => setTimeout(r, 150));
    });
    cleanup();
    t.equal(typeof received, 'number');
    t.end();
});
